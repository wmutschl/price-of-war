...
for i=1:npar
    ...
    hh_plt_prior = area(x2, f2, 'BaseValue', 0);
    set(hh_plt_prior, 'FaceColor', 'r', 'FaceAlpha', 0.2);
    %%hh_plt = plot(x2, f2, '-k', 'linewidth', 2);
    %%set(hh_plt, 'color', [0.7 0.7 0.7]);
    hold on;
    %%plot(x1, f1, '-k', 'linewidth', 2);
    % plot posterior as a filled area in light transparent blue (60% opacity)
    hh_plt_posterior = area(x1, f1, 'BaseValue', 0);
    set(hh_plt_posterior, 'FaceColor', [ 0.6843 0.8157 0.9182], 'FaceAlpha', 0.8);
    uistack(hh_plt_prior, 'bottom'); % ensure the prior is behind the posterior
    if ~issmc(options_) && ~options_.mh_posterior_mode_estimation
        %plot([pmod pmod], [0.0 1.1*top0], '--g', 'linewidth', 2);
        pmod_line = plot([pmod pmod], [0.0 1.1*top0], 'k--', 'linewidth', 1.5);
        set(pmod_line, 'Color', [0 0 0 0.6]);  % Set to black with 60% opacity
    end
    box on
    axis([borneinf bornesup 0 1.1*top0])
    set(gca,'FontSize',13);
    if TeX 
        title(texnam, 'Interpreter', 'latex','FontSize',18)
    else
        title(nam, 'Interpreter', 'none','FontSize',16)
    end
    hold off
    drawnow
